...
address.call()调用的任何函数，都会产生交易，消耗gas。即使调用的函数是pure修饰的函数。
技术上是允许的，没有意义
...
0.4.10 版之後，新增了 *require(), assert(), revert()* 三個函式。

編譯器會把 *require() 以及 revert()* 編譯成 *0xfd*。把 *assert()* 編譯成 *0xfe*。 

throw 處理方式跟 *require*相同，會編譯成 *0xfd*。
