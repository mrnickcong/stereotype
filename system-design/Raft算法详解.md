...
- [Raft算法详解](#raft算法详解)
  ...
  - [2. Raft算法角色](#2-raft算法角色)
    - [2.1 Raft算法角色有哪些](#21-raft算法角色有哪些)
    - [2.2 Raft算法角色状态转换](#22-raft算法角色状态转换)
    - [2.3 Raft算法角色任期term](#23-raft算法角色任期term)
  - [3. Leader选举](#3-leader选举)
    - [3.1 心跳触发Leader选举](#31-心跳触发leader选举)
    - [3.2 Leader选举过程](#32-leader选举过程)
  - [4. 日志同步](#4-日志同步)
    - [4.1 Raft日志同步过程](#41-raft日志同步过程)
    - [4.2 Raft日志结构](#42-raft日志结构)
    - [4.3 Raft日志同步主从一致性](#43-raft日志同步主从一致性)
  - [5. 安全性](#5-安全性)
    - [5.1 Raft如何保证安全性](#51-raft如何保证安全性)
    - [5.2 已提交的日志被覆盖](#52-已提交的日志被覆盖)
  - [6. 日志压缩](#6-日志压缩)
...
![Raft算法角色状态转换](./images/Raft算法角色状态转换.png)

Follower只响应其他服务器的请求。如果Follower超时没有收到Leader的消息，它会成为一个Candidate并且开始一次Leader选举。收到大多数服务器投票的Candidate会成为新的Leader。Leader在宕机之前会一直保持Leader的状态。

### 2.3 Raft算法角色任期term

![Raft算法任期term](./images/Raft算法任期term.png)
...
![Leader选举过程](./images/Leader选举过程.png)
...
![Leader和Followers上日志不一致](./images/Leader和Followers上日志不一致.png)
...
![已提交的日志被覆盖](./images/已提交的日志被覆盖.png)
