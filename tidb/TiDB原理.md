...
- [TiDB原理](#tidb原理)
  - [TiDB 简介](#tidb-简介)
    ...
    - [五大核心特性](#五大核心特性)
    - [四大核心应用场景](#四大核心应用场景)
  - [TiDB 数据库的存储](#tidb-数据库的存储)
    - [Key-Value Pairs（键值对）](#key-value-pairs键值对)
    - [本地存储 (RocksDB)](#本地存储-rocksdb)
    - [Raft 协议](#raft-协议)
    - [Region](#region)
    - [MVCC](#mvcc)
    - [分布式 ACID 事务](#分布式-acid-事务)
  - [TiDB 数据库的计算](#tidb-数据库的计算)
    - [数据与 Key-Value 的映射关系](#数据与-key-value-的映射关系)
      - [表数据与 Key-Value 的映射关系](#表数据与-key-value-的映射关系)
      - [索引数据和 Key-Value 的映射关系](#索引数据和-key-value-的映射关系)
      - [映射关系小结](#映射关系小结)
    - [Key-Value 映射关系示例](#key-value-映射关系示例)
    - [元信息管理](#元信息管理)
    - [SQL 层简介](#sql-层简介)
    - [SQL 运算](#sql-运算)
    - [分布式 SQL 运算](#分布式-sql-运算)
    - [SQL 层架构](#sql-层架构)
  - [TiDB 数据库的调度](#tidb-数据库的调度)
    - [场景描述](#场景描述)
    - [调度的需求](#调度的需求)
    - [调度的基本操作](#调度的基本操作)
    - [信息收集](#信息收集)
    - [调度的策略](#调度的策略)
      - [副本在 Store 之间的分布均匀分配](#副本在-store-之间的分布均匀分配)
      - [Leader 数量在 Store 之间均匀分配](#leader-数量在-store-之间均匀分配)
      - [访问热点数量在 Store 之间均匀分配](#访问热点数量在-store-之间均匀分配)
      - [各个 Store 的存储空间占用大致相等](#各个-store-的存储空间占用大致相等)
      - [控制调度速度，避免影响在线服务](#控制调度速度避免影响在线服务)
    - [调度的实现](#调度的实现)
...
- 它给出了一种兼容 MySQL 的 HTAP 数据库，支持强一致性，并且分布式可扩展。
- TiDB 实现为分层架构，其中 TiDB 服务器作为无状态计算层出于顶层。
- 底层存储层实现为支持事务的键值数据库，称为 TiKV。
- TiKV 的设计受到了 Google Spanner 的启发
...
![TiDB架构图](./images/TiDB架构图.png)

#### TiDB Server

![TiDBServer](./images/TiDBServer.png)
...
![Raft-in-TiDB](./images/Raft-in-TiDB.png)
...
![Region-in-TiDB](./images/Region-in-TiDB.png)
...
![Region-All-in-TiDB](./images/Region-All-in-TiDB.png)
...
![naive-sql-flow](./images/naive-sql-flow.png)
...
通过使用 `pd-ctl` 可以查看到 `TiKV Store` 的状态信息。

`TiKV Store` 的状态具体分为 `Up`，`Disconnect`，`Offline`，`Down`，`Tombstone`。

各状态的关系如下：

- Up：表示当前的 TiKV Store 处于提供服务的状态。
- Disconnect：当 PD 和 TiKV Store 的心跳信息丢失超过 20 秒后，该 Store 的状态会变为 
- Disconnect 状态，当时间超过 max-store-down-time 指定的时间后，该 Store 会变为 Down 状态。
...
![TiKV-store-status-relationship](./images/TiKV-store-status-relationship.png)
